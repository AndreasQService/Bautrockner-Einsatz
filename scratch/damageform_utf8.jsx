...
export default function DamageForm({ onCancel, initialData, onSave, mode = 'desktop', isDarkMode = true }) {
    ...
    // ÔöÇÔöÇ Auto-Sync: Pending Fotos hochladen wenn Netz zur├╝ckkommt ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    useEffect(() => {
        const syncPendingPhotos = async () => {
            if (isSyncing) return;
            setIsSyncing(true);

            console.log('[Sync] ­ƒöä Netz verf├╝gbar ÔÇô starte Sync ausstehender Fotos...');
            ...
            try {
                ...
                if (pending.length === 0) {
                    console.log('[Sync] Ô£à Keine ausstehenden Fotos.');
                ...
                console.log(`[Sync] Ô£à ${synced}/${pending.length} Fotos synchronisiert`);
        ...
        // Online-Event: automatisch syncen wenn Netz zur├╝ckkommt
        const handleOnline = () => {
            setIsOnline(true);
            console.log('[Netz] Ô£à Online ÔÇô starte Auto-Sync...');
            syncPendingPhotos();
        };
        const handleOffline = () => {
            setIsOnline(false);
            console.log('[Netz] ÔÜá´©Å Offline-Modus aktiv');
        };
    ...
    // ÔöÇÔöÇ OneDrive-Backfill: Supabase-Fotos ÔåÆ pers├Ânliches OneDrive ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    // L├ñuft einmal beim ├ûffnen eines Projekts. Findet alle Fotos die in Supabase
    // gesichert sind (storagePath vorhanden) aber noch nicht auf OneDrive sind
    // (oneDriveItemId fehlt). Download via Supabase Storage ÔåÆ Upload via User-Token.
    // Funktioniert auf jedem Ger├ñt, da Blobs per HTTP aus Supabase geladen werden.
    useEffect(() => {
        ...
        const backfill = async () => {
            ...
            console.log(`[OneDrive-Backfill] ­ƒöä ${missing.length} Fotos ohne OneDrive-Link ÔÇô starte Backfill...`);
            ...
            for (const img of missing) {
                try {
                    ...
                    if (dlErr || !blob) {
                        console.warn(`[OneDrive-Backfill] ÔÜá´©Å Download fehlgeschlagen: ${img.storagePath}`);
                        continue;
                    }

                    // Upload ins pers├Ânliche OneDrive
                    ...
                    if (odResult?.itemId || odResult?.odPath) {
                        ...
                        console.log(`[OneDrive-Backfill] Ô£à ${img.name} ÔåÆ OneDrive`);
                    }
                } catch (err) {
                    console.warn(`[OneDrive-Backfill] ÔÜá´©Å Fehler bei ${img.name}:`, err.message);
                }
            }
        };

        // 3s warten bis formData vollst├ñndig geladen ist
    ...
    const toggleDictation = (fieldId, contextItem = null) => {
        ...
        if (!SpeechRecognition) {
            alert("Ihr Browser unterst├╝tzt keine Spracherkennung (SpeechRecognition).");
            return;
        }
    ...
    const toggleFindingsListening = () => {
        ...
        if (!SR) { alert('Ihr Browser unterst├╝tzt keine Spracherkennung.'); return; }
        const rec = new SR();
        rec.lang = 'de-DE';
    ...
    const handleContactQR = (contact) => {
        ...
        const fields = {
            ...
            phone: contact.phone || '',
            email: contact.email || '',
            note: [contact.floor, contact.role].filter(Boolean).join(' ┬À '),
        };
    ...
    const startRecording = async (targetId = 'modal') => {
        ...
        try {
            const audioConstraints = {
                echoCancellation: true,   // Browser-Verarbeitung f├╝r sauberes Signal
                noiseSuppression: true,   // Hilft bei schwachem BT/AirPods-Signal
                autoGainControl: true,    // Boost f├╝r leise Mikrofone
            };
            if (selectedDeviceId) audioConstraints.deviceId = { exact: selectedDeviceId };
            const stream = await navigator.mediaDevices.getUserMedia({ audio: audioConstraints });

            // Beste MIME-Type f├╝r Browser-Kompatibilit├ñt (Whisper unterst├╝tzt webm, mp4, ogg)
        ...
        try {
            ...
            if (data.text) {
                ...
                if (cleanText === '' || cleanText === '.') {
                    ...
                    const sizeHint = audioBlob?.size < 5000 ? "\n\nMikrofon-Signal zu leise ÔÇô bitte Lautst├ñrke erh├Âhen." : "";
...
END:VCARD`;
    ...
    // --- Image Upload Handler: Supabase (Pflicht-Backup) + OneDrive (Prim├ñrarchiv) ---
    const handleImageUpload = async (files, contextData = {}) => {
        ...
        for (const file of files) {
            ...
            try {
                ...
                // ÔöÇÔöÇ SCHRITT 0: IndexedDB (sofort, offline, garantiert) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                if (!isDoc) {
                    try {
                        localPreviewUrl = await savePhotoLocally(tempId, formData.id || 'temp', file, {
                            subFolder, odFolder, contextData
                        });
                        // State sofort mit lokaler URL aktualisieren (kein Netz n├Âtig!)
                        setFormData(prev => ({
                            ...prev,
                            images: prev.images.map(img =>
                        ...
                        console.log('[IndexedDB] Ô£à Foto lokal gesichert:', tempId);
                    } catch (idbErr) {
                        console.warn('[IndexedDB] Lokaler Speicher fehlgeschlagen:', idbErr.message);
                    }
                }

                // ÔöÇÔöÇ SCHRITT 1: Supabase (immer ÔÇô garantierter Backup) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                if (supabase) {
                    try {
                        ...
                        if (!sbErr) {
                            ...
                            console.log('[Supabase] Ô£à Backup gespeichert:', fileName);

                            // IndexedDB-Status auf 'synced' ÔåÆ Badge-Counter geht auf 0
                ...
                // ÔöÇÔöÇ SCHRITT 1.5: OneDrive pers├Ânlich (direkt, via User-Token) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                if (!isDoc) {
                    try {
                        const uploadedOd = await uploadPhotoAndGetUrl(odFolder, subFolder, file);
                        if (uploadedOd?.itemId || uploadedOd?.odPath) {
                            odResult = uploadedOd;
                            console.log('[OneDrive] Ô£à Foto direkt hochgeladen:', uploadedOd.odPath);
                        }
                    } catch (odErr) {
                        console.warn('[OneDrive] Direkter Upload fehlgeschlagen (Journal ├╝bernimmt):', odErr.message);
                    }
                }

                // ÔöÇÔöÇ SCHRITT 2: Journal-Eintrag f├╝r Backend-Upload (Variante C) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                // Das Backend (Supabase Edge Function) ├╝bernimmt den
                ...
                if (!isDoc && supabasePath) {
                    try {
                        ...
                        if (sb) {
                            ...
                            console.log('[Variante C] Ô£à Journal-Eintrag erstellt ÔÇô Backend l├ñdt hoch');

                            // ÔöÇÔöÇ Sofortiger Worker-Trigger (fire & forget) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                            ...
                            fetch(
                                ...
                                if (r.ok) console.log('[Variante C] ­ƒÜÇ Worker sofort getriggert');
                            }).catch(() => { }); // Fire & forget ÔÇô niemals UI blockieren
                        }
                    } catch (jErr) {
                        // Journal-Fehler ist nicht kritisch ÔÇô Bild ist in Supabase Storage gesichert
                        console.warn('[Variante C] Journal-Fehler (Bild trotzdem gesichert):', jErr.message);
                    }
                }

                // ÔöÇÔöÇ SCHRITT 3: State mit beiden Pfaden aktualisieren ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                setFormData(prev => ({
                    ...prev,
                    images: prev.images.map(img =>
                        img.id === tempId ? {
                            ...img,
                            // Vorschau: OneDrive ÔåÆ Supabase ÔåÆ blob (Session)
                            preview: odResult?.downloadUrl || supabaseUrl || previewUrl,
                            previewFallback: supabaseUrl || previewUrl,
                            // OneDrive (permanent)
                            oneDriveItemId: odResult?.itemId || null,
                            oneDrivePath: odResult?.odPath || null,
                            // Supabase (tempor├ñrer Backup, periodisch bereinigt)
                            storagePath: supabasePath,
    ...
    // Cleanup in Variante C: Der Backend-Worker (Edge Function) ├╝bernimmt
    // das L├Âschen verifizierter Blobs nach der Sicherheitsfrist.
    ...
    const generatePDFExport = async (customFormData = null) => {
        ...
        try {
            console.log("[PDF Master] Nutze zentralen PDFService f├╝r Export...");
    ...
    const handleEmailImport = (data) => {
        ...
        setFormData(prev => {
            ...
            // --- ALTE Struktur (R├╝ckw├ñrtskompatibilit├ñt) ---
            ...
            // Fallback: KI hat Eigent├╝mer als Kontakt erkannt, aber nicht in rechnungs_details geschrieben
            if (!nextData.ownerName) {
                const eigKontakt = (data.kontakte || []).find(c => c.rolle === 'Eig.' || c.rolle === 'Eigent├╝mer' || c.rolle === 'Eig');
            ...
            const addOrMerge = (name, telefon, rolle, email, wohnung = '', contactPerson = '') => {
                ...
                // Rolle-K├╝rzel ÔåÆ Dropdown-Wert
                const rolleMap = { 'Handw.': 'Handwerker', 'Verw.': 'Verwaltung', 'Eig.': 'Eigent├╝mer', 'HW': 'Hauswart', 'Mieter': 'Mieter', 'AG': 'Auftraggeber', 'Auftraggeber': 'Auftraggeber' };
            ...
            // Neue Struktur: Block-├£berschrift = Kontaktrolle
            ...
            // Mieter: Array mit wohnung ÔåÆ floor
        ...
        // Geocoding-Fallback: Falls Strasse vorhanden aber PLZ/Ort fehlen ÔåÆ OpenStreetMap
            ...
            const geocodeStreet = async (street) => {
                try {
                    ...
                    if (geoData && geoData.length > 0) {
                        ...
                        if (plz || ort) {
                            console.log(`[Geocoding] ${street} ÔåÆ PLZ: ${plz}, Ort: ${ort}`);
                            setFormData(prev => ({
                                ...prev,
                                zip: prev.zip || plz,
                                city: prev.city || ort
    ...
    // ÔöÇÔöÇÔöÇ Stabile Callbacks f├╝r UploadPanel ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    const renderDocCard = (img, idx) => (
        ...
        >
            ...
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    if (!window.confirm(`Dokument "${img.name}" wirklich l├Âschen?`)) return;
                    setFormData(prev => ({
                        ...prev,
    ...
    // ÔöÇÔöÇ TECHNIKER MODUS: Kachel-Home & Messen-Auswahl ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    if (mode === 'technician' && (techTab === null || techTab === 'messung')) {
        const TECH_TILES = [
            { id: 'uebersicht', label: '├£bersicht', icon: '­ƒôï', color: '#3B82F6', status: null },
            { id: 'aufnahme', label: 'Schadenaufnahme', icon: '­ƒöì', color: '#F97316', status: 'Schadenaufnahme' },
            { id: 'leck', label: 'Leckortung', icon: '­ƒÆº', color: '#06B6D4', status: 'Leckortung' },
            { id: 'trocknung', label: 'Trocknung', icon: '­ƒî¼', color: '#A855F7', status: 'Trocknung' },
            { id: 'messung', label: 'Messung', icon: '­ƒôÅ', color: '#1E6DB7', status: null },
        ];
        const adresse = [formData.street, [formData.zip, formData.city].filter(Boolean).join(' ')].filter(Boolean).join(', ');
        const sub = [formData.projectNumber, formData.damageCategory].filter(Boolean).join(' ┬À ');
        return (
            <div className={isDarkMode ? "force-dark-mode" : ""} style={{ minHeight: '100vh', backgroundColor: 'var(--color-app-bg)', padding: '2rem 1.25rem 3rem', fontFamily: 'var(--font-desktop)', color: 'var(--text-main)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    {TECH_TILES.slice(0, 4).map(tile => (
                        <button key={tile.id} onClick={() => { 
                            ...
                            <span style={{ fontSize: '1rem', fontWeight: 700, color: isDarkMode ? 'var(--text-main)' : '#1E293B' }}>{tile.label === 'Trocknung' ? 'Ger├ñte' : tile.label}</span>
                        </button>
                    ))}
                </div>
                            ...
                            <div style={{ backgroundColor: 'var(--surface)', borderRadius: '16px', width: '100%', maxWidth: '400px', padding: '1.5rem', color: 'var(--text-main)', border: '1px solid var(--border)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        ...
                                        <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700 }}>
                                            {!activeApt || techRoomSelectorMode === 'geraete' ? 'Wohnung w├ñhlen' : activeApt}
                                        </h3>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '40vh', overflowY: 'auto', paddingBottom: '1rem' }}>
                                        {uniqueApartments.length > 0 ? uniqueApartments.map(apt => {
                                            ...
                                            const btnLabel = techRoomSelectorMode === 'geraete' 
                                                ? (aptDevices.length > 0 ? 'Ger├ñte anzeigen Ô×ö' : 'Ger├ñt hinzuf├╝gen Ô×ö')
                                                : 'R├ñume anzeigen Ô×ö';
                                            ...
                                            <div style={{ textAlign: 'center', color: '#94A3B8', padding: '1rem 0' }}>Noch keine R├ñume angelegt.</div>
                                        )}
                                    </div>
                                ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '40vh', overflowY: 'auto', paddingBottom: '1rem' }}>
                                        {allRooms.filter(r => (r.apartment || 'Allgemeiner Bereich').trim() === activeApt).map(r => (
                                            <button key={r.id} onClick={() => {
                                                ...
                                                <span style={{ color: '#94A3B8', fontSize: '0.8rem' }}>├ûffnen Ô×ö</span>
                                            </button>
                                        ))}
                                ...
                                <div style={{ marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.5rem' }}>
                                    <button onClick={() => {
                                        ...
                                        <Plus size={18} /> {techRoomSelectorMode === 'geraete' ? 'Ger├ñt hinzuf├╝gen' : 'Neuer Raum / Neue Messung'}
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <MeasurementModal
                    ...
                    apartments={[...new Set((formData.contacts || []).filter(c => c.role === 'Mieter' || c.role === 'Eigent├╝mer').map(c => [c.name, c.apartment].filter(Boolean).join(' - ').trim()).filter(Boolean))]}
        ...
        <>
            <div className="card" style={{ maxWidth: mode === 'desktop' ? '1000px' : '800px', margin: '0 auto', padding: mode === 'desktop' ? '1.5rem' : '1rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', paddingBottom: '1rem', borderBottom: '1px solid var(--border)' }}>
                    ...
                    {/* Schritt-Abschluss Checkbox ÔÇö nur Techniker, nicht ├£bersicht, nicht Trocknung */}
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setFormData(prev => {
                                                ...
                                                if (isChecked) {
                                                    const hasOwner = (newData.contacts || []).some(c => c.role === 'Eigent├╝mer');
                                                    if (!hasOwner) {
                                                        newData.contacts = [...(newData.contacts || []), { id: Date.now(), role: 'Eigent├╝mer', name: newData.ownerName || '', phone: '', email: newData.ownerEmail || '' }];
                                                    }
                                                } else {
                                                    newData.contacts = (newData.contacts || []).filter(c => c.role !== 'Eigent├╝mer');
                                                }
                                                return newData;
                                            });
                                        }}
                                        style={{ width: '16px', height: '16px', accentColor: 'var(--primary)' }}
                                    />
                                    Eigent├╝mer ist gleichzeitig Bewohner (Ansprechpartner vor Ort)
                                </label>
                            </div>
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setFormData(prev => {
                                                ...
                                                if (newData.ownerIsResident) {
                                                    newData.contacts = (newData.contacts || []).map(c => c.role === 'Eigent├╝mer' ? { ...c, name: val } : c);
                                                }
                                                return newData;
                                            });
                                        }}
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setFormData(prev => {
                                                ...
                                                if (newData.ownerIsResident) {
                                                    newData.contacts = (newData.contacts || []).map(c => c.role === 'Eigent├╝mer' ? { ...c, email: val } : c);
                                                }
                                                return newData;
                                            });
                                        }}
                    ...
                    <div style={{ marginBottom: '1.5rem', borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', backgroundColor: 'var(--surface)', padding: '0.5rem', borderRadius: '8px', border: '1px solid var(--border)' }}>
                            {formData.images && formData.images.filter(img => {
                                ...
                                return img && !img.roomId && !isDoc && img.assignedTo !== 'Schadenfotos' && img.assignedTo !== 'Messprotokolle' && img.assignedTo !== 'Pl├ñne';
                            }).map((img, idx) => (
                                <div key={idx}
                                    style={{
                                        position: 'relative',
                            ...
                            {(!formData.images || formData.images.filter(img => !img.roomId && img.assignedTo !== 'Schadenfotos' && img.assignedTo !== 'Messprotokolle' && img.assignedTo !== 'Pl├ñne' && !(img.type === 'document' || img.name?.toLowerCase().endsWith('.msg') || img.name?.toLowerCase().endsWith('.pdf') || img.name?.toLowerCase().endsWith('.txt'))).length === 0) && (
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', width: '100%', textAlign: 'center', padding: '0.5rem' }}>Keine Bilder vorhanden.</div>
                            )}

                            {/* Grosse Drag-and-Drop Zone ÔÇô volle Breite */}
                            <div style={{ width: '100%', marginTop: '0.5rem' }}>
                                <div
                                    ...
                                    title="Bilder hier ablegen oder klicken zum Ausw├ñhlen"
                                >
                                    ...
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', transition: 'color 0.2s', textAlign: 'center' }}>
                                        Bilder hier ablegen, <u>klicken</u> oder <kbd style={{ fontSize: '0.75rem', padding: '1px 4px', background: 'var(--border)', borderRadius: '3px' }}>Ctrl+V</kbd> zum Einf├╝gen
                                    </span>
                                </div>
                                <input
                ...
                <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', alignItems: 'stretch', marginBottom: '1.5rem' }}>
                    {/* Map Card ÔÇô Anzeige in Desktop und Techniker-├£bersicht */}
                    {(mode === 'desktop' || (mode === 'technician' && techTab === 'uebersicht')) && (
                        <div className="card" style={{ flex: '1 1 350px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                            ...
                            {(formData.street || formData.address) ? (
                                <div style={{ 
                                    ...
                                    <div style={{ flex: 1, height: '100%', position: 'relative' }}>
                                        ...
                                        {!formData.customMapImage && (
                                            <label
                                                ...
                                                title="Eigene Karte/Bild hinzuf├╝gen"
                                            >
                                                <Camera size={16} />
                                    ...
                                    {/* Rechte Seite: Das hinzugef├╝gte Bild (halbiert die Karte) */}
                                    {formData.customMapImage && (
                        ...
                        <>
                            ...
                            <div style={{
                                ...
                                {formData.contacts.map((contact, idx) => {
                                    ...
                                    if (contact.role === 'Eigent├╝mer' && !formData.ownerIsResident) return null;
                                    
                                    const roleColorMap = { 'Auftraggeber': '#1E6DB7', 'Verwaltung': '#1E6DB7', 'Eigent├╝mer': '#1E6DB7', 'Handwerker': '#1E6DB7', 'Hauswart': '#1E6DB7', 'Mieter': '#1E6DB7', 'Sonstiges': 'var(--text-muted)' };
                                    ...
                                    return (
                                        <div key={idx} className="glass-card" style={{
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '1.25rem 1.5rem 1.5rem' }}>
                                                ...
                                                {/* Row 1b: Ansprechperson ÔÇô nur bei Firma-Rollen */}
                                                {!['Mieter', 'Auftraggeber', 'Verwaltung', 'Eigent├╝mer'].includes(contact.role) && (
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                            ...
                            >
                                <h3 className="section-header" style={{ marginBottom: 0, border: 'none' }}>
                                    ...
                                    {!isRoomsExpanded && formData.rooms?.length > 0 && (
                                        <span style={{ fontSize: '0.75rem', fontWeight: 500, color: 'var(--text-muted)', marginLeft: '0.5rem' }}>
                                            ({formData.rooms.length} Raum{formData.rooms.length !== 1 ? '├ñume' : ''})
                                        </span>
                                    )}
                                </h3>
                                    ...
                                    <div style={{ marginBottom: '1.25rem' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
                                                    if (!SR) { alert('Diktat wird von diesem Browser nicht unterst├╝tzt.'); return; }
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                                            ...
                                            >
                                                ...
                                                {[...new Set([
                                                    ...
                                                    ...(formData.contacts || []).filter(c => c.role === 'Mieter' || c.role === 'Eigent├╝mer').map(c => c.name).filter(Boolean)
                                                ])].sort().map(apt => (
                                            ...
                                            {/* Freitext nur wenn "Neue Wohnung" gew├ñhlt */}
                        ...
                        <div key={room.id} className="card" style={{ 
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', flex: 1, minWidth: 0, paddingRight: '1rem', flexWrap: 'wrap' }}>
                                    ...
                                    {room.stockwerk && (
                                        <span style={{ fontWeight: 700, fontSize: '1rem', color: mode === 'technician' ? 'var(--text-muted)' : 'var(--primary)', opacity: 0.75, whiteSpace: 'nowrap' }}>ÔÇó {room.stockwerk}</span>
                                    )}
                                    {room.apartment && (
                                        <span style={{ fontWeight: 700, fontSize: '1rem', color: mode === 'technician' ? 'var(--text-muted)' : 'var(--primary)', opacity: 0.75, whiteSpace: 'nowrap' }}>ÔÇó Whg {room.apartment}</span>
                                    )}
                                </div>
                                <div style={{
                                    display: 'grid',
                                                                ...
                                                                >
                                                                    ...
                                                                    <div style={{ flex: '0 0 160px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
                                                                        ...
                                                                        {linkingImageId === img.id && (
                                                                            <div style={{ fontSize: '0.65rem', color: '#1E6DB7', fontWeight: 'bold', textAlign: 'center' }}>Bitte Ziel w├ñhlen...</div>
                                                                        )}
                                                                        <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', padding: '0 2px', alignItems: 'center' }}>
                                                                            <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', fontSize: '0.75rem', cursor: 'pointer', color: 'var(--text-main)' }}>
                                                                                <input
                                                                    ...
                                                                    {/* Actions: Delete, Link ÔÇô sortiere per Drag & Drop */}
                                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                                        {!thermalImg ? (
                                                                            <button
                                                                                type="button"
                                                                                className="btn btn-ghost"
                                                                                title="Als Thermobild verkn├╝pfen"
                    ...
                    <div className="card" style={{
                            ...
                            {[
                                ...
                                return (
                                    <div key={item} style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                                        ...
                                        {/* Titel + Textarea ÔÇö erscheint wenn angehakt */}
                                        {isChecked && (
                                            <div style={{
                                                ...
                                                {/* Titel ÔÇö entspricht sectionTitle im PDF: 16pt, blau, bold */}
                                                <div style={{
                                                    fontSize: '0.85rem',
                                                    fontWeight: 800,
                                                    color: '#1E6DB7',
                        ...
                        <div style={{ position: 'relative' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                                <label style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-muted)', fontWeight: 800 }}>Zus├ñtzliche Massnahmen</label>
                                <button
                                    type="button"
                                    className="btn-glass"
                                    onClick={() => toggleDictation('measures')}
                            ...
                            return (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                    {measuredRooms.map(({ room, entries }) => {
                                        ...
                                        const allImages = [
                                            ...(canvasImage ? [{ id: 'current', src: canvasImage, label: `Aktuell ┬À ${dateStr}`, isCurrent: true }] : [])
                                        ];
                                        ...
                                        return (
                                            <div key={room.id} style={{
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', fontSize: '0.85rem', color: 'var(--text-main)' }}>
                                                    <div><strong>Ger├ñt:</strong> {latestEntry.device || '-'}</div>
                    ...
                    <div style={{ marginBottom: '2rem', borderTop: '1px solid var(--border)', paddingTop: '1.5rem', ...(mode === 'desktop' ? { display: 'flex', flexDirection: 'column' } : {}) }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: 'var(--primary)' }}>
                            ...
                            {(mode === 'technician' && techSelectedEquipmentRoom?.apartment)
                                ? `Ger├ñte, Wohnung ${techSelectedEquipmentRoom.apartment}`
                                : 'Ger├ñte'}
                        </h2>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                        ...
                                        {!selectedDevice && newDevice.deviceNumber && (
                                            <>
                                                <input
                                                    type="text"
                                                    placeholder="Ger├ñtetyp (z.B. Kondenstrockner) *"
                                                    className="form-input"
                            ...
                            {showAddDeviceForm && mode === 'technician' && (
                                <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(4px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
                                    <div style={{ backgroundColor: 'var(--surface)', borderRadius: '16px', width: '100%', maxWidth: '400px', padding: '1.5rem', color: 'var(--text-main)', border: '1px solid var(--border)', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                            <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700 }}>Ger├ñt hinzuf├╝gen</h3>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                            <input
                                                type="text"
                                                placeholder="Ger├ñte-Nr. scannen oder tippen *"
                                                className="form-input"
                                                autoFocus
                                            ...
                                            <select
                                                ...
                                                style={{ fontSize: '1.1rem', padding: '1rem' }}
                                            >
                                                <option value="">Raum w├ñhlen... (Optional)</option>
                                                {formData.rooms
                                            ...
                                            >
                                                <div style={{ display:'flex', alignItems:'center', gap: '0.4rem', color:'#94A3B8', pointerEvents: 'none' }}>
                                                    ...
                                                    <span style={{ fontSize:'0.8rem', fontWeight:700 }}>
                                                        {activeNumpadField.field === 'counterStart' ? 'Z├ñhlerstand Start' : 'Laufzeit (Std.)'}
                                                    </span>
                                                </div>
                            ...
                            {Object.keys(unsubscribeStates).length > 0 && mode === 'technician' && (() => {
                                ...
                                return (
                                    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(4px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
                                        <div style={{ backgroundColor: 'var(--surface)', borderRadius: '16px', width: '100%', maxWidth: '400px', padding: '1.5rem', color: 'var(--text-main)', border: '1px solid var(--border)', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                                <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700 }}>Ger├ñt abmelden</h3>
                                                <button onClick={() => {
                                            ...
                                            <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '8px' }}>
                                                ...
                                                <div style={{ fontSize: '0.85rem', color: '#94A3B8', marginTop: '0.5rem', borderTop: '1px solid var(--border)', boxShadow: '0 -4px 20px rgba(0,0,0,0.04)', paddingTop: '0.5rem' }}>Start: {device.startDate} ÔÇó Z├ñhler: {device.counterStart} kWh</div>
                                            </div>

                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                                    <div>
                                                        <label style={{ fontSize: '0.8rem', color: '#94A3B8', display: 'block', marginBottom: '0.3rem', fontWeight: 600 }}>Z├ñhler Ende</label>
                                                        <input
                                                            type="number"
                                                            className="form-input"
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    {(formData.equipment || [])
                                                    ...
                                                    <div style={{ textAlign: 'right', minWidth: '80px' }}>
                                                        {isFinished ? (
                                                            <>
                                                                ...
                                                                <div style={{ fontSize: '0.75rem', color: '#475569', marginTop: '0.2rem' }}>{device.hours} h ÔÇó {getDaysDiff(device.startDate, device.endDate)} Tg.</div>
                                                            </>
                                                        ) : (
                                        ...
                                        {(formData.equipment || []).filter(device => !techSelectedEquipmentRoom?.apartment || (device.apartment || 'Allgemeiner Bereich').trim() === techSelectedEquipmentRoom.apartment).length === 0 && (
                                            <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Keine Ger├ñte vorhanden.</div>
                                        )}
                                        {(formData.equipment || []).filter(device => !techSelectedEquipmentRoom?.apartment || (device.apartment || 'Allgemeiner Bereich').trim() === techSelectedEquipmentRoom.apartment).length > 0 && (
                                ...
                                <div style={{ border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
                                    ...
                                    <tbody>
                                        ...
                                        {(formData.equipment || []).length === 0 && (
                                            <tr>
                                                <td colSpan="4" style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Keine Ger├ñte vorhanden.</td>
                                            </tr>
                                        )}
            ...
            {/* ÔöÇÔöÇ QR-Code Modal (mit editierbaren Feldern) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
            {qrModal && (
                <div
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#475569', textAlign: 'center' }}>
                            ­ƒô▒ Kamera-App ├Âffnen ┬À QR scannen ┬À Kontakt speichern
                        </div>
                        <button
                            onClick={() => setQrModal(null)}
                            style={{
