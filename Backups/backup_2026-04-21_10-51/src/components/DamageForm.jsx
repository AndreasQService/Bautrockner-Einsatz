...
// ─── Durable Upload Queue (neue Architektur) ───────────────────────────────
import { enqueueFiles } from '../lib/uploads/queueRepository';
import { runUploadWorker } from '../lib/uploads/uploadWorker';
import { reconcileProject } from '../lib/uploads/reconcile';
    ...
    const handleImageUpload = async (files, contextData = {}) => {
        ...
        for (const file of files) {
            ...
            try {
                ...
                // ── SCHRITT 2: OneDrive – durable Queue (Blob → IndexedDB → Upload-Session → Verifikation) ──
                if (!isDoc) {
                    try {
                        const remoteBasePath = `QTool/${odFolder}/Fotos/${subFolder.replace(/[^a-zA-Z0-9]/g, '_')}`;
                        const projectId = formData.id || formData.projectNumber || 'tmp';

                        // Bild in durable Queue schreiben (lokal persistiert, kein Netz nötig)
                        const queued = await enqueueFiles(projectId, [file], remoteBasePath);

                        if (queued[0]?.status === 'persisted') {
                            console.log('[DurableUpload] ✅ Bild lokal gesichert, Upload startet im Hintergrund:', queued[0].id);

                            // Upload-Worker im Hintergrund starten (kein await – blockiert UI nicht)
                            void runUploadWorker({ maxParallel: 2 }).then(async () => {
                                // Nach Worker: Reconciliation (Verifikation gegen OneDrive-Realität)
                                await reconcileProject(projectId);
                                console.log('[DurableUpload] 🔍 Reconciliation abgeschlossen für Projekt:', projectId);
                            }).catch(err => {
                                console.warn('[DurableUpload] Worker-Fehler (Blob bleibt gesichert):', err.message);
                            });

                            // odResult simulieren für State-Kompatibilität (wird durch reconcile überschrieben)
                            odResult = { itemId: null, odPath: remoteBasePath + '/' + queued[0].remoteFileName, downloadUrl: null };
                        } else if (queued[0]?.status === 'failed') {
                            // Blob konnte nicht gespeichert werden
                            console.warn('[DurableUpload] ⚠️ Persistierung fehlgeschlagen:', queued[0]?.errorMessage);
                        }
                        // Wenn kein Account → Worker startet, erkennt kein Token → bleibt in 'persisted'
                        // → klare Statusmeldung im UI über Upload-Queue sichtbar
                    } catch (odErr) {
                        console.warn('[DurableUpload] Queue-Fehler (Supabase-Backup aktiv):', odErr.message);
                    }
                }
