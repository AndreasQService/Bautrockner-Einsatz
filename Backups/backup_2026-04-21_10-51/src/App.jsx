...
function App() {
  ...
  // OneDrive: MSAL-Instanz registrieren + Silent-Check (KEIN automatischer Popup/Redirect)
  useEffect(() => {
    setMsalInstance(instance);
    const accounts = instance.getAllAccounts();
    if (accounts.length > 0) {
      // Nur silent Token-Erneuerung – kein interaktiver Login
      instance.acquireTokenSilent({
        scopes: ['Files.ReadWrite.All'],
        account: accounts[0],
      }).then(() => {
        console.log('[MSAL] ✅ Session wiederhergestellt:', accounts[0].username);
      }).catch(err => {
        // Silent fehlgeschlagen: Status = nicht verbunden, kein Popup
        console.info('[MSAL] Kein gültiger Token (nicht verbunden):', err.message);
      });
    }
    // Kein loginPopup / loginRedirect beim Start!
  }, [instance]);
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                ...
                {!isTechnicianMode && (
                  <>
                    ...
                    {/* OneDrive Status – dezenter Badge, kein Login-Button */}
                    <div
                      title={isAuthenticated
                        ? `OneDrive verbunden (${accounts[0]?.username || accounts[0]?.name || ''})`
                        : 'OneDrive nicht verbunden – Upload im Hintergrund nicht aktiv'}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '0.35rem',
                        ...
                        fontSize: '0.78rem',
                        color: isAuthenticated ? '#10B981' : 'var(--text-muted)',
