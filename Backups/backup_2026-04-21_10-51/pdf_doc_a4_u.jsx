    ...
    return (
        <Document>
            <Page size="A4" style={styles.page} wrap>
                    ...
                    <View style={{ marginBottom: 20, alignItems: 'flex-start' }} wrap={false}>
                        <View style={{ width: '100%' }}>
                            <View style={styles.imageContainer}>
                                ...
                                <Text style={styles.imageDescription}>Au├ƒenansicht / ├£bersicht</Text>
                            </View>
                        </View>
                        <View style={styles.divider} />
                    </View>
                ...
                {((data.description && data.includeDescriptionInReport !== false) || (data.images && data.images.some(img => !img.roomId && img.assignedTo !== 'Schadenfotos' && img.assignedTo !== 'Pl├ñne' && img.assignedTo !== 'Messprotokolle' && img.includeInReport !== false))) && (
                    <View style={{ marginBottom: 15 }}>
                        ...
                        {data.images && data.images.some(img => !img.roomId && img.assignedTo !== 'Schadenfotos' && img.assignedTo !== 'Pl├ñne' && img.assignedTo !== 'Messprotokolle' && img.includeInReport !== false) && (
                            <View style={{ marginTop: 10 }}>
                                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                                    {data.images.filter(img => !img.roomId && img.assignedTo !== 'Schadenfotos' && img.assignedTo !== 'Pl├ñne' && img.assignedTo !== 'Messprotokolle' && img.includeInReport !== false).map((img, i) => (
                                        ...
                                        <View style={styles.imageContainer}>
                                            {firstImage.preview ? (
                                                ...
                                                <View style={[styles.image, { alignItems: 'center', justifyContent: 'center' }]}>
                                                    <Text style={{ fontSize: 8, color: '#ef4444' }}>[ BILD NICHT VERF├£GBAR ]</Text>
                                                </View>
                                            )}
                ...
                {/* Pl├ñne & Grundrisse */}
                {data.images && data.images.some(img => img.assignedTo === 'Pl├ñne' && img.includeInReport !== false) && (
                    <View style={{ marginBottom: 20 }} wrap={false}>
                        <Text style={styles.sectionTitle}>PL├äNE & GRUNDRISSE</Text>
                        <View style={styles.imageGrid}>
                            {data.images.filter(img => img.assignedTo === 'Pl├ñne' && img.includeInReport !== false).map((img, i) => (
                                <View key={i} style={styles.imageContainer}>
                                    <Image src={img.preview} style={styles.image} />
                        ...
                        {data.selectedMeasures && data.selectedMeasures.length > 0 && (
                            <View style={{ marginBottom: data.measures ? 10 : 0 }}>
                                {data.selectedMeasures.map((measure, idx) => (
                                    <Text key={idx} style={[styles.textBlock, { marginBottom: 2 }]}>ÔÇó {measure}</Text>
                                ))}
                            </View>
                        )}
                ...
                <View style={styles.footer} fixed>
                    <View style={styles.footerContent}>
                        <View>
                            <Text style={styles.footerText}>Q-Service AG ÔÇó Kriesbachstrasse 30 ÔÇó 8600 D├╝bendorf</Text>
                            <Text style={styles.footerText}>www.q-service.ch ÔÇó Tel. 043 819 14 18</Text>
                        </View>
