-- ======================================================================
-- DAUERHAFTE BEREINIGUNG FÜR SUPABASE-TESTPROJEKT (QTool-Test)
-- 
...
-- 
-- DAUERHAFTE VERSION: ENDET MIT COMMIT
-- ======================================================================
...
BEGIN
  ...
  RAISE NOTICE '[PRE-COMMIT CHECK PASSED] Alle Datenbank-Bedingungen vor COMMIT exakt verifiziert.';
END $$;

-- ----------------------------------------------------------------------
-- COMMIT DAUERHAFTE BEREINIGUNG
-- ----------------------------------------------------------------------
COMMIT;

-- ----------------------------------------------------------------------
-- 7. POST-COMMIT VERIFIKATION (REIN LESEND)
-- ----------------------------------------------------------------------
SELECT jsonb_build_object(
  ...
  'database_cleanup_status', CASE 
    WHEN (SELECT count(*) FROM public.damage_reports) = 0
     ...
     AND EXISTS(SELECT 1 FROM storage.buckets WHERE id = 'case-files') = true
     AND EXISTS(SELECT 1 FROM storage.buckets WHERE id = 'damage-images') = true
     AND EXISTS(SELECT 1 FROM storage.buckets WHERE id = 'project-images') = true
     AND (SELECT count(*) FROM storage.objects) = 0
     THEN 'VERIFIED_SUCCESS'
    ELSE 'VERIFICATION_FAILED'
  END,
  'bucket_cleanup_status', 'MANUAL_ACTION_REQUIRED'
) AS post_commit_verification;
