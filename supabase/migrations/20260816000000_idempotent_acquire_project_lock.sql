-- Migration: 20260816000000_idempotent_acquire_project_lock.sql
-- Makes acquire_project_lock and create_project_and_acquire_lock 100% idempotent.
-- Includes DROP FUNCTION IF EXISTS to allow Postgres return type replacement.
...
BEGIN
  ...
  -- 2. Idempotent Handling: If session token currently holds a lock (same or different project),
  -- update open_project_id seamlessly instead of throwing an error.
  IF FOUND THEN
    IF v_request_session.open_project_id IS NOT NULL
       AND v_request_session.open_project_id IS DISTINCT FROM p_project_id THEN
      -- Release previous project lock for this session token
  ...
  -- 3. Check if another session token holds the project lock
  ...
  IF FOUND THEN
    -- If owned by the same session token, extend lease and return success
    IF v_owner.session_token = p_session_token THEN
      UPDATE public.project_sessions
         SET device = p_device, client_id = p_client_id, last_seen = now(), owner_user_id = v_request_uid
    ...
    -- Blocked by another device/session token
  ...
  -- 4. Upsert lock for this session token
...
BEGIN
  ...
  -- Idempotent handling: clear old project lock for this session token if different
