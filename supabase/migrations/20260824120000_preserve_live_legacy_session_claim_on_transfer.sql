...
begin
  ...
  -- Serialize requests from the same browser session before locking the target
  -- project. Updating the single session row below transfers its lock without
  -- ever allowing the token to own two projects at once.
  ...
  -- Preserve the deployed compatibility path for an old, already released
  -- session row which predates authenticated ownership metadata.
