...
begin
  ...
  -- Serialize every request made with this browser session before locking the
  -- target project. The session row itself guarantees that one token can own
  -- only one project; updating that row below transfers ownership atomically.
