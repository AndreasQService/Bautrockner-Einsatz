-- ======================================================================
-- QTool Testumgebung: Konsolidiertes Schema (test_schema_init.sql)
-- Target: Nur für getrenntes Test-Supabase-Projekt (QTool-Test)
-- ⚠️ VERBOTENES LIVE-PROJEKT: yxdoecdqttgdncgbzyus
-- ⚠️ PFLICHT-TEST-PROJEKT:   aoxduqspiezzyqeqyzzl
--
-- STRICT SECURITY: NO ANON ACCESS / AUTHENTICATED ONLY
-- NO STORAGE BUCKET WRITES / NO DROP TABLES
-- RECONCILED WITH CODEBASE COLUMNS & CHECK CONSTRAINTS
-- ======================================================================
...
-- ----------------------------------------------------------------------
-- 2. CORE TABLE: damage_reports & COLUMN RECONCILIATION
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 3. DEVICE & CATALOG TABLES & RECONCILIATION
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 4. QUEUE, SESSIONS & RECONCILED STORAGE_STATUS CONSTRAINT
-- ----------------------------------------------------------------------
...
-- Reconciled Check Constraint including 'remote_uploaded_manifest_pending'
...
-- ----------------------------------------------------------------------
-- 5. WORKFLOW, TASKS & STATUS HISTORY
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 7. ROOMS, MEASUREMENTS & PROTOCOLS
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 9. ROW LEVEL SECURITY (RLS) ACTIVATION ON ALL 16 TABLES
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 10. STRICT AUTHENTICATED POLICIES (NO ANON ACCESS ALLOWED)
-- ----------------------------------------------------------------------
...
-- ----------------------------------------------------------------------
-- 11. STORAGE OBJECT POLICIES (NO BUCKET CREATION OR TABLE WRITES)
-- ----------------------------------------------------------------------
...
CREATE POLICY "Auth select case-files" ON storage.objects
  FOR SELECT TO authenticated USING (bucket_id = 'case-files' AND auth.uid() IS NOT NULL);

CREATE POLICY "Auth insert case-files" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'case-files' AND auth.uid() IS NOT NULL);

CREATE POLICY "Auth update case-files" ON storage.objects
  FOR UPDATE TO authenticated USING (bucket_id = 'case-files' AND auth.uid() IS NOT NULL) WITH CHECK (bucket_id = 'case-files' AND auth.uid() IS NOT NULL);

CREATE POLICY "Auth delete case-files" ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'case-files' AND auth.uid() IS NOT NULL);
