  ...
  return createPortal(
    <div ref={ref} style={st} onClick={e => e.stopPropagation()}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
        <span style={{ fontWeight: 700, fontSize: "0.85rem", color: "var(--text-main, #1E293B)" }}>{step.icon} {step.label}</span>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted, #64748B)", padding: 0, display: "flex" }}><X size={14} /></button>
      </div>
      ...
      {mode === "date" && <div style={{ marginBottom: "0.65rem" }}><label style={{ fontSize: "0.68rem", color: "var(--text-muted, #64748B)", display: "block", marginBottom: "0.25rem", fontWeight: 600 }}>Fälligkeit</label><input type="date" value={date} onChange={e => setDate(e.target.value)} autoFocus style={{ width: "100%", padding: "0.4rem 0.55rem", borderRadius: 8, border: `1px solid ${date ? "rgba(59,130,246,0.5)" : "var(--border, #E2E8F0)"}`, background: "var(--background, #F8FAFC)", color: "var(--text-main, #1E293B)", fontSize: "0.82rem", boxSizing: "border-box", outline: "none" }} /></div>}
